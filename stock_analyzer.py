...
class StockAnalyzer:
    ...
    def __init__(self, initial_cash=1000000):
        ...
        self.GOOGLE_API_MODEL = os.getenv("GOOGLE_API_MODEL", 'gemini-2.0-flash-exp')
    ...
    def get_stock_data(self, stock_code, market_type='A', start_date=None, end_date=None):
        ...
        try:
            # 使用可靠的数据获取器，支持多数据源备选
            self.logger.info(f"使用可靠数据获取器获取 {stock_code} 数据")
            df = get_reliable_stock_data(stock_code, market_type, start_date, end_date)
            
            # 如果可靠数据获取器失败，回退到原有方法
            if df is None or df.empty:
                self.logger.warning(f"可靠数据获取器失败，回退到原有方法")
                if market_type == 'A':
    ...
    def perform_enhanced_analysis(self, stock_code, market_type='A'):
        ...
        enhanced_report = {
            ...
            'recommendation': {
                'action': self.get_recommendation(
