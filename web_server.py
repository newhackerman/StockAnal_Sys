...
print('开始AI分析')
scenario_predictor = ScenarioPredictor(analyzer)
...
def start_stock_analysis():
    ...
    try:
        ...
        if task['status'] == TASK_COMPLETED and 'result' in task:
            ...
            # 即使是缓存结果，也尝试保存到数据库（如果还没有保存的话）
            if USE_DATABASE:
                try:
                    save_analysis_result_to_db(stock_code, market_type, task['result'])
                except Exception as db_error:
                    app.logger.error(f"保存缓存结果到数据库失败: {str(db_error)}")
        ...
        if is_new:
            ...
            def run_analysis():
                ...
                if USE_DATABASE:
                    try:
                        save_analysis_result_to_db(stock_code, market_type, result)
                    except Exception as db_error:
                        app.logger.error(f"保存分析结果到数据库失败: {str(db_error)}")
...
def api_qa_clear():
    ...
    try:
        ...
        # 清除对话历史
        result = stock_qa.clear_conversation(conversation_id=conversation_id, stock_code=stock_code)

        return custom_jsonify(result)
    except Exception as e:
        app.logger.error(f"清除对话历史出错: {traceback.format_exc()}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/qa/capabilities', methods=['GET'])
def api_qa_capabilities():
    """获取AI模型能力信息API"""
    try:
        capabilities = stock_qa.get_model_capabilities()
        
        return custom_jsonify({
            'success': True,
            'capabilities': capabilities,
            'model_info': {
                'name': stock_qa.openai_model,
                'provider': stock_qa.API_PROVIDER
            }
        })
    except Exception as e:
        app.logger.error(f"获取模型能力信息出错: {traceback.format_exc()}")
        return jsonify({'error': str(e)}), 500
...
def save_analysis_result_to_db(stock_code, market_type, analysis_result):
    """保存分析结果到数据库"""
    if not USE_DATABASE:
        app.logger.info("数据库功能未启用，跳过保存")
        return
    
    app.logger.info(f"开始保存分析结果到数据库: {stock_code}, {market_type}")
    
    session = get_session()
    try:
        # 提取基本信息
        scores = analysis_result.get('scores', {})
        # 兼容两种字段名：total_score 和 total
        total_score = scores.get('total_score', scores.get('total', 0))
        recommendation = analysis_result.get('recommendation', {}).get('action', '')
        
        app.logger.info(f"提取的信息 - 评分: {total_score}, 建议: {recommendation}")
        
        # 删除同一只股票的旧记录，只保留最新的
        existing_records = session.query(AnalysisResult).filter(
            AnalysisResult.stock_code == stock_code,
            AnalysisResult.market_type == market_type
        ).all()
        
        if existing_records:
            app.logger.info(f"删除股票 {stock_code} 的 {len(existing_records)} 条旧记录")
            for record in existing_records:
                session.delete(record)
            session.flush()  # 立即执行删除，但不提交
        ...
        analysis_record = AnalysisResult(
            ...
            analysis_data=json.dumps(analysis_result, ensure_ascii=False, default=str),
        ...
        app.logger.info(f"分析结果已成功保存到数据库: {stock_code}, ID: {analysis_record.id}")
        
    except Exception as e:
        session.rollback()
        app.logger.error(f"保存分析结果到数据库失败: {str(e)}")
...
def get_analysis_history():
    ...
    try:
        ...
        try:
            ...
            # 添加过滤条件
            if stock_code:
                query = query.filter(AnalysisResult.stock_code.ilike(f'%{stock_code}%'))
            
            if market_type:
                query = query.filter(AnalysisResult.market_type == market_type)
            
            if date_range:
                days = int(date_range)
                cutoff_date = datetime.now() - timedelta(days=days)
                query = query.filter(AnalysisResult.analysis_date >= cutoff_date)
            ...
            if sort == 'date_desc':
                ...
                query = query.order_by(AnalysisResult.total_score.desc())
            elif sort == 'score_asc':
                query = query.order_by(AnalysisResult.total_score.asc())
            ...
            for record in records:
                try:
                    ...
                    history_item = {
                        ...
                        'analysis_date': record.analysis_date.isoformat() if record.analysis_date else '',
                    ...
                    app.logger.error(f"解析历史记录数据时出错: {str(e)}")
                    continue
        ...
        app.logger.error(f"获取历史分析记录时出错: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
@app.route('/api/analysis_history/<int:record_id>', methods=['GET'])
def get_analysis_detail(record_id):
    ...
    try:
        ...
        session = get_session()
        try:
            record = session.query(AnalysisResult).filter(AnalysisResult.id == record_id).first()
        ...
        app.logger.error(f"获取分析记录详情时出错: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
@app.route('/api/analysis_history/<int:record_id>', methods=['DELETE'])
def delete_analysis_record(record_id):
    ...
    try:
        ...
        app.logger.error(f"删除分析记录时出错: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
@app.route('/analysis_detail/<int:record_id>')
def analysis_detail_page(record_id):
    """分析记录详情页面"""
    try:
        if not USE_DATABASE:
            return render_template('error.html', 
                                 error_message='数据库功能未启用',
                                 error_code=400)
        ...
        app.logger.error(f"显示分析记录详情页面时出错: {str(e)}")
        return render_template('error.html', 
                             error_message=f'加载详情页面失败: {str(e)}',
                             error_code=500)
